...
(function() {
  ...
  // 피드백에 답장 보내기 (notice 발송 + 여러 feedback 한번에 resolved 처리)
  // feedbackIds: 그 사람이 보낸 모든 피드백 ID 배열. 한번에 다 resolved 처리.
  async function replyToFeedback({ feedbackIds, recipientEmail, title, body }) {
    try {
      // 1) 사내 메모 발송 (target_emails로 그 사람만)
      const noticeR = await sendNotice({
        type: 'notice',
        title,
        body,
        fromName: '유버디 (Buddy)',
        popup: true,  // 피드백 답장은 입장 시 팝업으로 띄워 안 놓치게
        targetEmails: [recipientEmail],
      });
      if (!noticeR.ok || !noticeR.notice) {
        throw new Error('notice send failed: ' + (noticeR.error || ''));
      }
      // 2) 답장 기록만 박음. resolved 는 따로 건드리지 않음.
      //    "답장 보냄" 과 "실제 해결됨" 은 다른 상태 (버그/기능 반영 후 별도 마킹).
