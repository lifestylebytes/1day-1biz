// ============================================================
// Supabase Client, 1day-1biz 공용 helper
// onboarding.html, operator.html, mainboard.html 모두 이 파일 로드
// ============================================================
//
// 사용:
//   <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
//   <script src="lib/supabase-config.js"></script>
//   <script src="lib/supabase-client.js"></script>
//
// 그러면 window.OD = { signup, updateProgress, fetchAllUsers, ... } 사용 가능
// ============================================================

(function() {
  // 설정 없으면 조용히 비활성화 (localStorage 모드만 동작)
  const cfg = window.__SUPABASE_CONFIG;
  if (!cfg || !cfg.url || !cfg.anonKey || cfg.url.includes('xxxxxxxx')) {
    console.log('[Supabase] config not set, running in localStorage-only mode');
    window.OD = { enabled: false };
    return;
  }

  // Supabase JS SDK가 로드돼있는지
  if (typeof supabase === 'undefined' || !supabase.createClient) {
    console.warn('[Supabase] SDK not loaded, add <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>');
    window.OD = { enabled: false };
    return;
  }

  const client = supabase.createClient(cfg.url, cfg.anonKey);

  // ============================================================
  // AI 사수 피드백 (Edge Function mentor-feedback 호출)
  // 실패하면 {ok:false} 반환 → 앱은 규칙 기반 피드백만 보여줌 (폴백).
  // 8초 타임아웃, 느리면 포기하고 규칙 기반 유지.
  // ============================================================
  // 카나리: 평소엔 운영 함수(mentor-feedback)를 부르고,
  // 내 브라우저에서만 staging 함수로 미리 확인하고 싶을 때 아래 하나로 켠다.
  //   URL 한 번:  ?mentorfn=staging   (끄려면 ?mentorfn=prod)
  //   고정:       localStorage.setItem('__OD_MENTOR_FN','staging')  (끄려면 'prod' 또는 removeItem)
  // 일반 사용자는 플래그가 없으니 항상 운영 함수만 호출 = 영향 0.
  function _mentorFnName() {
    try {
      const q = new URLSearchParams(location.search).get('mentorfn');
      if (q === 'staging' || q === 'prod') {
        localStorage.setItem('__OD_MENTOR_FN', q);
      }
      const flag = localStorage.getItem('__OD_MENTOR_FN');
      return flag === 'staging' ? 'mentor-feedback-staging' : 'mentor-feedback';
    } catch (e) {
      return 'mentor-feedback';
    }
  }

  async function getMentorAiFeedback(payload) {
    try {
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 8000);
      // 서버 일일 상한(사람별)용 식별자. 로그인 이메일을 같이 보낸다 (YB-SEC-002).
      let _em = '';
      try { _em = String((JSON.parse(localStorage.getItem('__OD_USER') || '{}') || {}).email || '').toLowerCase(); } catch (e) {}
      const { data, error } = await client.functions.invoke(_mentorFnName(), {
        body: Object.assign({}, payload, _em ? { email: _em } : {}),
      });
      clearTimeout(timer);
      if (error) return { ok: false, error: error.message };
      return data || { ok: false };
    } catch (e) {
      return { ok: false, error: e.message };
    }
  }

  // ============================================================
  // 가입, onboarding 끝에 호출
  // ============================================================
  // ============================================================
  // 초대 코드 검증, pilot_codes 테이블에 등록된 활성 코드인지 확인
  // 클라이언트 정규식은 'PILOT-XXXX' 같은 placeholder도 통과시키므로
  // 진짜 발급된 코드인지는 반드시 서버에서 확인해야 한다.
  // ============================================================
  async function checkInviteCode(code) {
    if (!code || typeof code !== 'string') return { ok: true, valid: false };
    try {
      const { data, error } = await client
        .from('pilot_codes')
        .select('code, label, cohort, expires_at, max_uses, used_count')
        .eq('code', code)
        .eq('active', true)
        .maybeSingle();
      if (error) throw error;
      if (!data) return { ok: true, valid: false };

      // 만료일 체크
      if (data.expires_at && new Date(data.expires_at).getTime() < Date.now()) {
        return { ok: true, valid: false, reason: 'expired', label: data.label };
      }
      // 사용 횟수 체크 (max_uses 설정돼있을 때만)
      if (typeof data.max_uses === 'number' && typeof data.used_count === 'number'
          && data.used_count >= data.max_uses) {
        return { ok: true, valid: false, reason: 'exhausted', label: data.label };
      }
      return { ok: true, valid: true, label: data.label, cohort: data.cohort };
    } catch (e) {
      console.warn('[Supabase] checkInviteCode failed:', e.message);
      // 서버 연결 실패 시 보수적으로 invalid 처리, 통과 안 시킴
      return { ok: false, valid: false, error: e.message };
    }
  }

  // ★ 매직 링크 발송 (비밀번호 없는 인증)
  // 동일 함수가 OTP 6자리 코드도 같이 발송함. 이메일 템플릿에서 {{ .Token }} 쓰면 코드 노출.
  async function sendMagicLink(email) {
    if (!email) return { ok: false, error: '이메일이 필요해요.' };
    try {
      const { data, error } = await client.auth.signInWithOtp({
        email: email.trim(),
        options: {
          emailRedirectTo: window.location.origin + '/auth-callback.html',
          shouldCreateUser: true,  // public.users 화이트리스트는 클라이언트에서 fetchByEmail로 가드
        },
      });
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] sendMagicLink failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 6자리 OTP 코드 검증 → 세션 생성 (어느 기기에서든 동일 코드로 로그인 가능)
  async function verifyOtpCode(email, code) {
    if (!email || !code) return { ok: false, error: '이메일과 코드가 필요해요.' };
    try {
      const { data, error } = await client.auth.verifyOtp({
        email: email.trim(),
        token: String(code).trim(),
        type: 'email',
      });
      if (error) throw error;
      return { ok: true, session: data.session, user: data.user };
    } catch (e) {
      console.warn('[Supabase] verifyOtpCode failed:', e.message);
      // Supabase 에러 메시지 친절하게 변환
      const msg = String(e.message || '');
      let friendly = e.message;
      if (/invalid|incorrect|expired/i.test(msg)) {
        friendly = '코드가 일치하지 않거나 만료됐어요. 메일 다시 확인해주세요.';
      } else if (/rate|too many/i.test(msg)) {
        friendly = '시도 횟수가 많아 잠시 차단됐어요. 1-2분 후 다시 시도해주세요.';
      }
      return { ok: false, error: friendly };
    }
  }

  // ★ 비밀번호로 로그인 (이메일+비밀번호)
  // 비번을 정한 적 없는 사용자는 'Invalid login credentials'가 나므로
  // 호출부에서 "비밀번호 설정/찾기" 또는 "이메일 코드"로 유도한다.
  async function signInWithPassword(email, password) {
    if (!email || !password) return { ok: false, error: '이메일과 비밀번호가 필요해요.' };
    try {
      const { data, error } = await client.auth.signInWithPassword({
        email: String(email).trim().toLowerCase(),
        password: String(password),
      });
      if (error) throw error;
      return { ok: true, session: data.session, user: data.user };
    } catch (e) {
      const msg = String(e.message || '');
      let friendly = e.message;
      if (/invalid login credentials/i.test(msg)) {
        friendly = '이메일 또는 비밀번호가 일치하지 않아요. 비밀번호를 정한 적이 없거나 잊으셨다면 아래 \'이메일 코드로 로그인\' 버튼을 눌러주세요.';
      } else if (/email not confirmed/i.test(msg)) {
        friendly = '이메일 인증이 먼저 필요해요. 이메일 코드로 로그인해주세요.';
      } else if (/rate|too many/i.test(msg)) {
        friendly = '시도가 많아 잠시 차단됐어요. 1-2분 후 다시 시도해주세요.';
      }
      return { ok: false, error: friendly };
    }
  }

  // ★ 가입 시 비밀번호로 Auth 계정 생성 (Confirm email OFF면 즉시 비번 로그인 가능)
  async function signUpWithPassword(email, password) {
    if (!email || !password) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.auth.signUp({
        email: String(email).trim().toLowerCase(),
        password: String(password),
      });
      if (error) throw error;
      return { ok: true, session: data.session, user: data.user };
    } catch (e) {
      console.warn('[Supabase] signUpWithPassword failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ 비밀번호 설정/변경 (현재 세션 필요)
  // 사용 흐름: 이메일 코드(OTP)로 인증해 세션을 만든 직후 호출 → updateUser로 비번 저장.
  // 이렇게 하면 "비번 잊음"도 이메일 코드만 받으면 새 비번을 다시 정할 수 있다 (영구 잠김 없음).
  async function setPassword(password) {
    if (!password || String(password).length < 8) {
      return { ok: false, error: '비밀번호는 8자 이상으로 정해주세요.' };
    }
    try {
      const { data: { session } } = await client.auth.getSession();
      if (!session) {
        return { ok: false, error: '인증 세션이 없어요. 이메일 코드 인증을 먼저 해주세요.' };
      }
      const { data, error } = await client.auth.updateUser({ password: String(password) });
      if (error) throw error;
      return { ok: true, user: data.user };
    } catch (e) {
      const msg = String(e.message || '');
      let friendly = e.message;
      if (/different from the old|should be different|same.*password/i.test(msg)) {
        friendly = '기존 비밀번호와 다른 비밀번호로 정해주세요.';
      } else if (/weak|at least|characters|length/i.test(msg)) {
        friendly = '비밀번호가 너무 약해요. 8자 이상으로 정해주세요.';
      }
      return { ok: false, error: friendly };
    }
  }

  // ★ 콘텐츠 자동수정: 승인된 편집(앱이 SCENARIOS에 덮어씀) 읽기
  async function getApprovedContentEdits() {
    try {
      const { data, error } = await client
        .from('content_edits').select('day,field,new_value').eq('status', 'approved');
      if (error) throw error;
      return { ok: true, edits: data || [] };
    } catch (e) {
      console.warn('[Supabase] getApprovedContentEdits failed:', e.message);
      return { ok: false, edits: [] };
    }
  }
  // 운영자: pending 제안 조회
  async function getPendingContentEdits(email) {
    try {
      const { data, error } = await client.rpc('get_pending_content_edits', {
        p_email: String(email || '').toLowerCase().trim(),
      });
      if (error) throw error;
      return { ok: true, edits: data || [] };
    } catch (e) {
      console.warn('[Supabase] getPendingContentEdits failed:', e.message);
      return { ok: false, edits: [], error: e.message };
    }
  }
  // 운영자: 승인/거부
  async function decideContentEdit(id, email, approve) {
    try {
      const { data, error } = await client.rpc('decide_content_edit', {
        p_id: id, p_email: String(email || '').toLowerCase().trim(), p_approve: !!approve,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] decideContentEdit failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ 승급(정직원 전환) 영속화. SECURITY DEFINER RPC라 세션/RLS 무관하게 확실히 기록.
  async function completeLevelup(email, score) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('complete_levelup', {
        p_email: String(email).toLowerCase().trim(),
        p_score: (score == null ? null : Number(score)),
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] completeLevelup failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // Day 60 대리 트랙 설문: 이메일당 1회만 기록 (기기/횟수 무관, RPC가 원자적으로 중복 차단)
  async function submitSeniorSurvey({ email, name, message, context }) {
    try {
      const { data, error } = await client.rpc('submit_senior_survey', {
        p_email: email ? String(email).toLowerCase().trim() : null,
        p_name: name || null,
        p_message: message || '',
        p_context: context || {},
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] submitSeniorSurvey failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ 웹 푸시 구독 저장 (PWA 알림). subscription = PushSubscription.toJSON()
  async function savePushSubscription(email, subscription) {
    if (!email || !subscription) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('save_push_subscription', {
        p_email: String(email).toLowerCase().trim(),
        p_subscription: subscription,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] savePushSubscription failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ 웹 푸시 구독 해제 (알림 끄기)
  async function deletePushSubscription(endpoint) {
    if (!endpoint) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('delete_push_subscription', { p_endpoint: endpoint });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] deletePushSubscription failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 현재 세션 가져오기 (매직 링크로 인증된 경우 토큰 반환)
  async function getCurrentSession() {
    try {
      const { data: { session } } = await client.auth.getSession();
      return { ok: true, session };
    } catch (e) {
      return { ok: false, session: null };
    }
  }

  // 로그아웃 (Supabase 세션도 같이 종료)
  async function signOut() {
    try {
      await client.auth.signOut();
      return { ok: true };
    } catch (e) {
      return { ok: false, error: e.message };
    }
  }

  // ★ 원자적 시나리오 저장 (덮어쓰기 방지)
  // 서버에서 기존 데이터 + 신규 entry 를 union merge.
  // 동시에 learning_history 테이블에 영구 기록 (append-only).
  async function saveScenarioAtomic(email, entry) {
    if (!email || !entry) return { ok: false };
    try {
      const { data, error } = await client.rpc('merge_scenario_completion', {
        p_email: email,
        p_entry: entry,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] saveScenarioAtomic failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ 원자적 노트 저장 (중복 제거 + history 보존)
  async function saveNoteAtomic(email, day, note) {
    if (!email || !day || !note) return { ok: false };
    try {
      const { data, error } = await client.rpc('merge_saved_note', {
        p_email: email,
        p_day: String(day),
        p_note: note,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] saveNoteAtomic failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ history에서 사용자 데이터 재구성 (응급 복구용)
  async function rebuildFromHistory(email) {
    try {
      const { data, error } = await client.rpc('rebuild_user_from_history', {
        p_email: email,
      });
      if (error) throw error;
      return { ok: true, scenarios: data || [] };
    } catch (e) {
      console.warn('[Supabase] rebuildFromHistory failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 가입 성공 시 호출, 코드별 사용 카운트 +1 (RPC, security definer)
  async function incrementCodeUsage(code) {
    if (!code) return;
    try {
      await client.rpc('increment_pilot_code_used', { p_code: code });
    } catch (e) {
      console.warn('[Supabase] incrementCodeUsage failed:', e.message);
    }
  }

  async function signup(profile) {
    try {
      // 가입 자체는 자유, 실제 사옥(mainboard) 입장 권한은
      // simulator.html 페이월에서 checkInviteCode + unlock 플래그로 별도 검증한다.

      // ★ 중복 가입 방지, upsert(덮어쓰기) 대신 insert로 변경.
      // 같은 이메일이 이미 있으면 서버에서 거부, 진도 데이터 보호.
      // 클라이언트(onboarding)에서도 사전 가드(fetchByEmail) 한 번 더 함.

      // localStorage 형식 → Supabase 컬럼 매핑
      const row = {
        email: profile.email,
        name: profile.name,
        signup_date: profile.signupDate,
        agreed_at: profile.agreedAt || profile.signupDate,
        emp_number: profile.empNumber,
        department: profile.department,
        mentor: profile.mentor,
        coworkers: profile.coworkers || [],
        level: profile.level,
        unlocked: !!profile.unlocked,
        day_in_company: profile.dayInCompany || 1,
        scenarios_completed: profile.scenariosCompleted || [],
        words_studied: profile.wordsStudied || [],
        production_attempts: profile.productionAttempts || { correct: 0, total: 0 },
        novelty_events: profile.noveltyEvents || [],
        last_active: new Date().toISOString(),
        total_sessions: 1,
        preferences: profile.preferences,
        onboarding_motivation: profile.onboardingMotivation,
        cohort: profile.cohort,
        membership_ends_at: profile.membershipEndsAt || profile.membership_ends_at || null,
        is_pilot_hire: !!profile.isPilotHire,
        is_dev_mode: !!profile.isDevMode,
        is_tester: !!profile.isTester,
        is_operator: !!profile.isOperator,
      };

      // ★ .select().single() 안 함, RLS가 anon에게 SELECT 권한 안 줘서 방금 넣은 row도 못 읽음.
      //   INSERT 만 시도, 에러 없으면 성공으로 본다.
      const { error } = await client
        .from('users')
        .insert(row);

      if (error) {
        const isDup = error.code === '23505'
          || /duplicate|already exists|unique/i.test(error.message || '');
        if (isDup) {
          return {
            ok: false,
            duplicate: true,
            error: '이미 가입된 이메일이에요. 메인 화면에서 \"다시 출근하기\"를 이용해주세요.',
          };
        }
        throw error;
      }

      // signup 이벤트 로그는 user.id 가 필요한데 못 읽으니 생략 (이벤트는 나중에 매직링크 로그인 후 기록)
      console.log('[Supabase] signup saved:', row.email);
      return { ok: true, user: row };
    } catch (e) {
      console.warn('[Supabase] signup failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ============================================================
  // 진도 업데이트, Day 변경, 시나리오 완료 시
  // ============================================================
  async function updateProgress(email, patch) {
    try {
      const { data, error } = await client
        .from('users')
        .update({
          ...patch,
          last_active: new Date().toISOString(),
        })
        .eq('email', email)
        .select()
        .single();

      if (error) throw error;
      return { ok: true, user: data };
    } catch (e) {
      console.warn('[Supabase] update failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 멤버십 동기화 전용 (CSV 도구): last_active 안 건드림 (마지막 학습 기록 오염 방지)
  async function syncMembership(email, patch) {
    if (!email || !patch) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client
        .from('users')
        .update(patch)
        .eq('email', email)
        .select('email, cohort, membership_ends_at, membership_cancel_at, unlocked')
        .single();
      if (error) throw error;
      return { ok: true, user: data };
    } catch (e) {
      console.warn('[Supabase] syncMembership failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ============================================================
  // 이벤트 로그, 학습 활동 기록
  // ============================================================
  async function logEvent(userId, type, payload = {}) {
    try {
      const { error } = await client
        .from('events')
        .insert({ user_id: userId, type, payload });
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] event log failed:', e.message);
      return { ok: false };
    }
  }

  // ============================================================
  // 운영자 뷰, 모든 사용자 가져오기
  // ============================================================
  async function fetchAllUsers() {
    try {
      const { data, error } = await client
        .from('users')
        .select('*')
        .order('signup_date', { ascending: false });
      if (error) throw error;
      return { ok: true, users: data };
    } catch (e) {
      console.warn('[Supabase] fetch users failed:', e.message);
      return { ok: false, users: [], error: e.message };
    }
  }

  // ============================================================
  // 운영자 뷰, 통계 (operator_stats view)
  // ============================================================
  async function fetchStats() {
    try {
      const { data, error } = await client
        .from('operator_stats')
        .select('*')
        .single();
      if (error) throw error;
      return { ok: true, stats: data };
    } catch (e) {
      console.warn('[Supabase] fetch stats failed:', e.message);
      return { ok: false, stats: null };
    }
  }

  // ============================================================
  // 공지 발송
  // ============================================================
  async function sendNotice({ type, title, body, fromName, popup, targetEmails }) {
    try {
      const row = {
        type, title, body,
        from_name: fromName || '유버디 (Buddy)',
        popup: !!popup,
      };
      // target_emails: null = 전체 공지, 배열 = 그 이메일들에게만 보임
      if (Array.isArray(targetEmails) && targetEmails.length > 0) {
        row.target_emails = targetEmails.map(e => String(e).toLowerCase().trim());
      }
      const { data, error } = await client
        .from('notices')
        .insert(row)
        .select()
        .single();
      if (error) throw error;
      return { ok: true, notice: data };
    } catch (e) {
      console.warn('[Supabase] notice send failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function updateNotice(id, { type, title, body, popup }) {
    try {
      const patch = {};
      if (type !== undefined)  patch.type = type;
      if (title !== undefined) patch.title = title;
      if (body !== undefined)  patch.body = body;
      if (popup !== undefined) patch.popup = !!popup;
      const { data, error } = await client
        .from('notices')
        .update(patch)
        .eq('id', id)
        .select()
        .single();
      if (error) throw error;
      return { ok: true, notice: data };
    } catch (e) {
      console.warn('[Supabase] update notice failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function fetchNotices(limit = 50, email = null) {
    try {
      let query = client.from('notices').select('*');
      if (email) {
        // 본인용 공지(target_emails에 본인 이메일 포함) + 전체 공지(target_emails NULL)만
        const safeEmail = String(email).toLowerCase().trim();
        query = query.or(`target_emails.is.null,target_emails.cs.{${safeEmail}}`);
      }
      query = query.order('sent_at', { ascending: false }).limit(limit);
      const { data, error } = await query;
      if (error) throw error;
      return { ok: true, notices: data };
    } catch (e) {
      console.warn('[Supabase] fetch notices failed:', e.message);
      return { ok: false, notices: [] };
    }
  }

  // 피드백에 답장 보내기 (notice 발송 + 여러 feedback에 답장 기록 + optional deadline)
  // feedbackIds: 그 사람이 보낸 모든 피드백 ID 배열
  // deadline: 마감일(YYYY-MM-DD), 있으면 같이 박힘. resolved는 안 건드림.
  async function replyToFeedback({ feedbackIds, recipientEmail, title, body, deadline }) {
    try {
      const noticeR = await sendNotice({
        type: 'notice',
        title, body,
        fromName: '유버디 (Buddy)',
        popup: true,
        targetEmails: [recipientEmail],
      });
      if (!noticeR.ok || !noticeR.notice) {
        throw new Error('notice send failed: ' + (noticeR.error || ''));
      }
      const ids = (Array.isArray(feedbackIds) ? feedbackIds : [feedbackIds]).filter(Boolean);
      if (ids.length > 0) {
        const patch = {
          reply_notice_id: noticeR.notice.id,
          replied_at: new Date().toISOString(),
        };
        if (deadline) patch.deadline = deadline;  // YYYY-MM-DD 형식
        const { error: fbErr } = await client
          .from('feedback')
          .update(patch)
          .in('id', ids);
        if (fbErr) throw fbErr;
      }
      return { ok: true, notice: noticeR.notice, count: ids.length };
    } catch (e) {
      console.warn('[Supabase] replyToFeedback failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 피드백 작업 완료 (notice 발송 + resolved 마킹 + completion 정보 기록)
  async function completeFeedback({ feedbackIds, recipientEmail, title, body }) {
    try {
      const noticeR = await sendNotice({
        type: 'notice',
        title, body,
        fromName: '유버디 (Buddy)',
        popup: true,  // 작업 완료도 팝업으로 알림 (사용자가 빠르게 인지)
        targetEmails: [recipientEmail],
      });
      if (!noticeR.ok || !noticeR.notice) {
        throw new Error('notice send failed: ' + (noticeR.error || ''));
      }
      const ids = (Array.isArray(feedbackIds) ? feedbackIds : [feedbackIds]).filter(Boolean);
      if (ids.length > 0) {
        const { error: fbErr } = await client
          .from('feedback')
          .update({
            resolved: true,
            completion_notice_id: noticeR.notice.id,
            completed_at: new Date().toISOString(),
          })
          .in('id', ids);
        if (fbErr) throw fbErr;
      }
      return { ok: true, notice: noticeR.notice, count: ids.length };
    } catch (e) {
      console.warn('[Supabase] completeFeedback failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 피드백 마감일만 따로 업데이트 (답장 안 보내고 deadline만 변경/제거)
  async function setFeedbackDeadline(id, deadline) {
    try {
      const { error } = await client
        .from('feedback')
        .update({ deadline: deadline || null })
        .eq('id', id);
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] setFeedbackDeadline failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function deleteNotice(id) {
    try {
      const { error } = await client
        .from('notices')
        .delete()
        .eq('id', id);
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] delete notice failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ============================================================
  // 대기 등록 (waitlist)
  // ============================================================
  async function saveWaitlistEntry(entry) {
    try {
      const row = {
        name: entry.name,
        email: entry.email,
        phone: entry.phone || null,
        prior_cohorts: entry.priorCohorts || [],
        current_env: entry.currentEnv || null,
        english_usage: entry.englishUsage || [],
        study_methods: entry.studyMethods || [],
        pain_points: entry.painPoints || [],
        goal: entry.goal || null,
        goal_detail: entry.goalDetail || null,
        message: entry.message || null,
        heard_from: entry.heardFrom || null,
        source: entry.source || "waitlist-page",
        user_agent: entry.userAgent || null,
      };

      const { data, error } = await client
        .from('waitlist')
        .insert(row)
        .select()
        .single();

      if (error) throw error;

      console.log('[Supabase] waitlist saved:', data.email);
      return { ok: true, entry: data };
    } catch (e) {
      console.warn('[Supabase] waitlist save failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function fetchWaitlist() {
    try {
      const { data, error } = await client
        .from('waitlist')
        .select('*')
        .order('created_at', { ascending: false });
      if (error) throw error;
      return { ok: true, entries: data };
    } catch (e) {
      console.warn('[Supabase] fetch waitlist failed:', e.message);
      return { ok: false, entries: [] };
    }
  }

  // ============================================================
  // ★ 멤버십 퇴직 처리 (구독 취소)
  // 사용자에게 "인수인계 기간" daysGrace(기본 30일) 부여 후 자동 만료.
  // 만료일 이후 mainboard 진입 시 simulator.html?expired=1 로 차단.
  // ============================================================
  async function cancelMembership(email, daysGrace = 30) {
    if (!email) return { ok: false, error: '이메일이 필요해요.' };
    try {
      const now = new Date();
      // daysGrace 0 = 오늘이 마지막 출근일 (그날 KST 23:59:59 까지). 앱은 만료 후 2일 유예를 두고 차단한다.
      const g = Number.isFinite(Number(daysGrace)) ? Math.max(0, Number(daysGrace)) : 30;
      const kstDay = new Date(now.getTime() + 9 * 3600e3); kstDay.setUTCDate(kstDay.getUTCDate() + g);
      const ends = new Date(kstDay.toISOString().slice(0, 10) + 'T23:59:59+09:00');
      const { data, error } = await client
        .from('users')
        .update({
          membership_cancel_at: now.toISOString(),
          membership_cancel_reason: 'operator_cancel',   // 운영자 뷰가 "퇴직 처리됨" 으로 구분하는 표시
          membership_ends_at: ends.toISOString(),
          last_active: now.toISOString(),
        })
        .eq('email', email)
        .select()
        .single();
      if (error) throw error;
      return { ok: true, user: data, endsAt: ends.toISOString() };
    } catch (e) {
      console.warn('[Supabase] cancelMembership failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ 퇴직 취소 (복직, 인수인계 기간 중 마음 바뀐 경우)
  async function restoreMembership(email) {
    if (!email) return { ok: false, error: '이메일이 필요해요.' };
    try {
      const { data, error } = await client
        .from('users')
        .update({
          membership_cancel_at: null,
          membership_cancel_reason: null,
          membership_ends_at: null,
          last_active: new Date().toISOString(),
        })
        .eq('email', email)
        .select()
        .single();
      if (error) throw error;
      return { ok: true, user: data };
    } catch (e) {
      console.warn('[Supabase] restoreMembership failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ============================================================
  // 이메일 존재 여부만 확인 (RLS 우회용, SECURITY DEFINER RPC 호출)
  // anon 상태에서 fetchByEmail 못 쓰는 케이스에 사용:
  //   - LoginScreen: 매직링크 보내기 전 가입자인지 확인
  //   - signup 중복 가드: 새 이메일이 이미 가입돼있는지 확인
  // ============================================================
  async function emailExists(email) {
    if (!email) return { ok: true, exists: false };
    try {
      const { data, error } = await client.rpc('email_exists', { p_email: email.trim() });
      if (error) throw error;
      return { ok: true, exists: !!data };
    } catch (e) {
      console.warn('[Supabase] emailExists failed:', e.message);
      return { ok: false, exists: false, error: e.message };
    }
  }

  // ============================================================
  // 본인 사용자 정보 (이메일로 lookup)
  // ============================================================
  // ★ RLS 우회 멤버십 동기화 (2026-08-18): 로그인 세션이 없거나 만료돼 users 직접 조회가
  //   RLS에 막혀도, 이메일+사번이 일치하면 본인 프로필을 돌려주는 SECURITY DEFINER RPC.
  //   (만료 잠금이 localStorage 동기화에 의존하는데, 그 동기화가 조용히 실패해서
  //    구독 종료자가 계속 fail-open으로 이용하던 구멍의 근본 수정. 박은실/최은정 케이스)
  async function fetchMembershipStatus(email, emp) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('get_membership_status', {
        p_email: email, p_emp: emp || null,
      });
      if (error) throw error;
      const row = Array.isArray(data) ? data[0] : data;
      return row ? { ok: true, user: row } : { ok: false, error: 'not found' };
    } catch (e) {
      console.warn('[Supabase] fetchMembershipStatus failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ★ TF 트랙 상태 저장 (2026-08-18): users 직접 UPDATE는 RLS에 막힐 수 있어 RPC 경로 사용.
  async function saveTfState(email, emp, track, startedAt, done) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { error } = await client.rpc('save_tf_state', {
        p_email: email, p_emp: emp || null,
        p_track: track || null,
        p_started_at: startedAt || null,
        p_done: Array.isArray(done) ? done : null,
      });
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] saveTfState failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function fetchByEmail(email) {
    try {
      const { data, error } = await client
        .from('users')
        .select('*')
        .eq('email', email)
        .maybeSingle();
      if (error) throw error;
      return { ok: true, user: data };
    } catch (e) {
      console.warn('[Supabase] fetch by email failed:', e.message);
      return { ok: false, user: null };
    }
  }

  // ============================================================
  // ★ 정규화 테이블 저장·조회 헬퍼 (2026-05-19 추가, RLS 사고 대응)
  // submissions / submissions_history / notes / journals
  // 모두 RPC만 통로. write-through verify로 저장 성공 확인.
  // ============================================================

  // 시나리오 영작 저장 + verify
  // payload.status === 'draft' 면 answer_text 빈 문자열도 허용 (mc 선택만 저장하는 케이스 포함)
  async function saveSubmission(email, day, payload) {
    if (!email || !day || !payload) {
      return { ok: false, error: 'invalid args' };
    }
    const isSubmitted = (payload.status || 'submitted') === 'submitted';
    if (isSubmitted && !payload.answer_text) {
      return { ok: false, error: 'submitted requires answer_text' };
    }
    try {
      const { data, error } = await client.rpc('save_submission', {
        p_email: email,
        p_day: Number(day),
        p_payload: payload,
      });
      if (error) throw error;

      // write-through verify, 제출 상태일 때만 본문 일치 확인 (draft는 mc만 갱신할 수 있음)
      let verified = true;
      if (isSubmitted) {
        const { data: verify, error: vErr } = await client.rpc('get_user_day', {
          p_email: email,
          p_day: Number(day),
        });
        verified = !vErr
          && verify && verify.submission
          && verify.submission.answer_text === payload.answer_text;
      }
      return { ok: true, verified: !!verified, result: data };
    } catch (e) {
      console.warn('[Supabase] saveSubmission failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 단어 노트 저장 (1row = 1note)
  async function saveNoteRow(email, day, text, label, kind) {
    if (!email || !day || !text) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('save_note', {
        p_email: email,
        p_day: Number(day),
        p_text: text,
        p_label: label || null,
        p_kind: kind || null,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] saveNoteRow failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 단어 노트 삭제 (email, day, text 로 정확히 1행)
  async function deleteNoteRow(email, day, text) {
    if (!email || !day || !text) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('delete_note', {
        p_email: email,
        p_day: Number(day),
        p_text: text,
      });
      if (error) throw error;
      return { ok: true, deleted: data };
    } catch (e) {
      console.warn('[Supabase] deleteNoteRow failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 업무일지 저장
  async function saveJournalRow(email, day, text) {
    if (!email || !day || !text) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('save_journal', {
        p_email: email,
        p_day: Number(day),
        p_text: text,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] saveJournalRow failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 운영자 조회, 전체 진도
  async function fetchUserProgress(email) {
    if (!email) return { ok: false, rows: [] };
    try {
      const { data, error } = await client.rpc('get_user_progress', { p_email: email });
      if (error) throw error;
      return { ok: true, rows: data || [] };
    } catch (e) {
      console.warn('[Supabase] fetchUserProgress failed:', e.message);
      return { ok: false, rows: [], error: e.message };
    }
  }

  // 운영자 조회, 특정 일자 풀 데이터 (시나리오 + 노트 + 일지 + history)
  async function fetchUserDay(email, day) {
    if (!email || !day) return { ok: false };
    try {
      const { data, error } = await client.rpc('get_user_day', {
        p_email: email,
        p_day: Number(day),
      });
      if (error) throw error;
      return { ok: true, data };
    } catch (e) {
      console.warn('[Supabase] fetchUserDay failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 운영자 뷰 · 계정별 진도 현황: 모든 계정의 완주 Day 목록 (email + day 숫자만)
  async function fetchProgressOverview() {
    try {
      const { data, error } = await client.rpc('get_progress_overview');
      if (error) throw error;
      const map = {};
      (Array.isArray(data) ? data : []).forEach(r => { if (r && r.email) map[String(r.email).toLowerCase()] = (r.done || []).map(Number); });
      return { ok: true, map };
    } catch (e) {
      console.warn('[Supabase] fetchProgressOverview failed:', e.message);
      return { ok: false, map: {}, error: e.message };
    }
  }

  // ★ cross-device hydrate, 사용자 전체 학습 데이터 한 번에 가져오기
  // 반환: { submissions:[...], notes:[...], journals:[...], notice_reads:[ids] }
  async function fetchUserFullData(email) {
    if (!email) return { ok: false };
    try {
      const { data, error } = await client.rpc('get_user_full_data', { p_email: email });
      if (error) throw error;
      return { ok: true, data };
    } catch (e) {
      console.warn('[Supabase] fetchUserFullData failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 데이터 헬스 카운터 (운영 대시보드용)
  async function fetchUserHealth(email) {
    if (!email) return { ok: false };
    try {
      const { data, error } = await client.rpc('get_user_health', { p_email: email });
      if (error) throw error;
      return { ok: true, health: data };
    } catch (e) {
      console.warn('[Supabase] fetchUserHealth failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // Day 진도 저장 (RPC)
  async function saveDayProgress(email, day) {
    if (!email || !day) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('save_day_progress', {
        p_email: email,
        p_day: Number(day),
      });
      if (error) throw error;
      return { ok: true, user: data };
    } catch (e) {
      console.warn('[Supabase] saveDayProgress failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 설문 응답 저장 (Day 2 1일차 피드백 등 범용)
  async function saveSurveyResponse(email, surveyKey, responses) {
    if (!email || !surveyKey || !responses) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('save_survey_response', {
        p_email: email,
        p_key: surveyKey,
        p_responses: responses,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] saveSurveyResponse failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 일정 체크 상태 저장 (한 day의 task 객체 union merge)
  async function saveTaskProgress(email, day, tasks) {
    if (!email || !day || !tasks) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('save_task_progress', {
        p_email: email,
        p_day: Number(day),
        p_tasks: tasks,
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] saveTaskProgress failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 카톡 출근 알림 설정 (2026-09-03). phone 은 빈 문자열이면 기존 값 유지, hour null 이면 끔
  async function setKakaoNotify(email, phone, hour) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('set_kakao_notify', {
        p_email: email,
        p_phone: phone || null,
        p_hour: (hour === null || hour === undefined || hour === '') ? null : Number(hour),
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] setKakaoNotify failed:', e.message);
      return { ok: false, error: e.message };
    }
  }
  async function getKakaoNotify(email) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('get_kakao_notify', { p_email: email });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] getKakaoNotify failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // ── 리텐션 엔진 (2026-09-14): EXP 이벤트 적재 + 주간 순위판. RPC 없으면 조용히 실패. ──
  async function addExpEvent(email, key, exp, kind, day, at) {
    if (!email || !key) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('add_exp_event', {
        p_email: email, p_key: String(key), p_exp: Number(exp) || 0, p_kind: kind || '', p_day: day == null ? null : Number(day), p_at: at || new Date().toISOString(),
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }
  async function fetchWeeklyBoard(limit) {
    try {
      let em = null; try { em = (JSON.parse(localStorage.getItem('__OD_USER') || 'null') || {}).email || null; } catch (e) {}
      const { data, error } = await client.rpc('get_weekly_board', { p_email: em, p_limit: Number(limit) || 50 });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }

  // 출근 날짜 복원 (2026-09-22): 서버 exp_events 의 실제 완주 시각 + 방어 인정일. 새 기기에서 날짜를 지어내지 않게.
  async function fetchActivityDates(email) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('get_activity_dates', { p_email: email });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }

  // 리텐션 상태 기기 간 동기화 (2026-09-24): 내 목표·아이템·복지P·결근 판정·출근 인정일. 키별 {v, at}
  async function getRetState(email) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('get_ret_state', { p_email: email });
      if (error) throw error;
      return { ok: true, result: data || {} };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }
  async function setRetState(email, patch) {
    if (!email || !patch) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('set_ret_state', { p_email: email, p_patch: patch });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }

  // 복지몰 이벤트 코드 (2026-09-24) + 이메일 지정 지급 수령
  async function redeemWpCode(email, code) {
    if (!email || !code) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('redeem_wp_code', { p_email: email, p_code: String(code) });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }
  async function claimWpGrants(email) {
    if (!email) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('claim_wp_grants', { p_email: email });
      if (error) throw error;
      return { ok: true, result: Array.isArray(data) ? data : [] };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }

  async function peekDaySentences(email, day, n) {
    try {
      const { data, error } = await client.rpc('peek_day_sentences', { p_email: email || null, p_day: Number(day), p_n: Number(n) || 3 });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) { return { ok: false, error: e && e.message }; }
  }

  // 오늘 출근한 동료 (마스킹된 이름만 내려옴). RPC 없으면 조용히 실패.
  async function getOfficeToday(limit) {
    try {
      const { data, error } = await client.rpc('get_office_today', { p_limit: Number(limit) || 10 });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      return { ok: false, error: e && e.message };
    }
  }

  // 공지 읽음 마킹 (RPC)
  async function markNoticeRead(email, noticeKey) {
    if (!email || !noticeKey) return { ok: false, error: 'invalid args' };
    try {
      const { data, error } = await client.rpc('mark_notice_read', {
        p_email: email,
        p_notice_key: String(noticeKey),
      });
      if (error) throw error;
      return { ok: true, result: data };
    } catch (e) {
      console.warn('[Supabase] markNoticeRead failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 사용자가 읽은 공지 ID 목록 (페이지 진입 시 hydrate)
  async function fetchNoticeReads(email) {
    if (!email) return { ok: false, keys: [] };
    try {
      const { data, error } = await client.rpc('get_notice_reads', { p_email: email });
      if (error) throw error;
      return { ok: true, keys: (data || []).map(r => r.notice_key) };
    } catch (e) {
      console.warn('[Supabase] fetchNoticeReads failed:', e.message);
      return { ok: false, keys: [], error: e.message };
    }
  }

  // 피드백 (사용자 → 운영자)
  // ★ insert 후 .select() 안 함: feedback SELECT는 운영자(is_op)만 허용이라
  //   일반 사용자가 방금 넣은 행을 되읽으면 RLS에 막혀 "new row violates RLS" 에러가 남.
  //   return=minimal로 넣기만 하고 되읽지 않음.
  async function submitFeedback({ email, name, day, category, message, context }) {
    if (!message || !message.trim()) return { ok: false, error: 'empty_message' };
    try {
      const { error } = await client
        .from('feedback')
        .insert({
          email: email || null,
          name: name || null,
          day: day || null,
          category: category || 'general',
          message: message.trim(),
          context: context || null,
        });
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] submitFeedback failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function fetchFeedback(limit = 100) {
    try {
      const { data, error } = await client
        .from('feedback')
        .select('*')
        .order('created_at', { ascending: false })
        .limit(limit);
      if (error) throw error;
      return { ok: true, feedback: data };
    } catch (e) {
      console.warn('[Supabase] fetchFeedback failed:', e.message);
      return { ok: false, feedback: [] };
    }
  }

  // 운영자 시트 뷰: Day1 설문(survey_responses) 전체 (운영자 이메일 검증 RPC, 읽기 전용)
  async function fetchSurveyResponses(operatorEmail, limit = 1000) {
    if (!operatorEmail) return { ok: false, rows: [], error: 'no operator email' };
    try {
      const { data, error } = await client.rpc('list_survey_responses', { p_email: operatorEmail, p_limit: limit });
      if (error) throw error;
      return { ok: true, rows: data || [] };
    } catch (e) {
      console.warn('[Supabase] fetchSurveyResponses failed:', e.message);
      return { ok: false, rows: [], error: e.message };
    }
  }

  async function setFeedbackResolved(id, resolved) {
    try {
      const { error } = await client
        .from('feedback')
        .update({ resolved: !!resolved })
        .eq('id', id);
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] setFeedbackResolved failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  async function deleteFeedback(id) {
    try {
      const { error } = await client.from('feedback').delete().eq('id', id);
      if (error) throw error;
      return { ok: true };
    } catch (e) {
      console.warn('[Supabase] deleteFeedback failed:', e.message);
      return { ok: false, error: e.message };
    }
  }

  // 공지별 확인(읽음) 사람 수 집계 (운영자용). notice_key 기준 distinct email 수.
  async function fetchNoticeReadCounts() {
    try {
      const { data, error } = await client.rpc('get_notice_read_counts');
      if (error) throw error;
      const map = {};
      (data || []).forEach(r => { map[r.notice_key] = Number(r.cnt) || 0; });
      return { ok: true, counts: map };
    } catch (e) {
      console.warn('[Supabase] fetchNoticeReadCounts failed:', e.message);
      return { ok: false, counts: {} };
    }
  }

  // ============================================================
  // ★ Outbox, DB write 실패 시 queue → 재시도. localStorage가 큐 저장소.
  // 페이지 진입 + online 이벤트 + 60초 간격으로 flush.
  // ============================================================
  const OUTBOX_KEY = '__OD_OUTBOX';

  function _readOutbox() {
    try { return JSON.parse(localStorage.getItem(OUTBOX_KEY) || '[]'); } catch (e) { return []; }
  }
  function _writeOutbox(arr) {
    try { localStorage.setItem(OUTBOX_KEY, JSON.stringify(arr)); } catch (e) {}
    _renderOutboxBadge();
  }
  function _renderOutboxBadge() {
    try {
      const q = _readOutbox();
      const el = document.getElementById('__od_outbox_badge');
      if (!el) return;
      if (!q.length) { el.style.display = 'none'; return; }
      el.style.display = 'block';
      el.textContent = '저장 대기 ' + q.length + '건 (자동 재시도 중)';
    } catch (e) {}
  }
  function pushOutbox(op) {
    const q = _readOutbox();
    q.push({ ...op, queuedAt: Date.now() });
    _writeOutbox(q);
  }
  async function flushOutbox() {
    const q = _readOutbox();
    if (!q.length) return { ok: true, drained: 0 };
    const remaining = [];
    let drained = 0;
    for (const op of q) {
      let r;
      try {
        switch (op.type) {
          case 'submission': r = await saveSubmission(op.email, op.day, op.payload); break;
          case 'note':       r = await saveNoteRow(op.email, op.day, op.text, op.label, op.kind); break;
          case 'journal':    r = await saveJournalRow(op.email, op.day, op.text); break;
          case 'day':        r = await saveDayProgress(op.email, op.day); break;
          case 'task':       r = await saveTaskProgress(op.email, op.day, op.tasks); break;
          case 'notice':     r = await markNoticeRead(op.email, op.noticeKey); break;
          default:           r = { ok: false, error: 'unknown op' };
        }
      } catch (e) { r = { ok: false, error: e.message }; }
      if (r && r.ok) drained++;
      else remaining.push(op);
    }
    _writeOutbox(remaining);
    return { ok: true, drained, remaining: remaining.length };
  }

  // 부팅 시 1회 + online 이벤트 + 60초 주기
  setTimeout(() => { _renderOutboxBadge(); flushOutbox().catch(() => {}); }, 1500);
  window.addEventListener('online', () => { flushOutbox().catch(() => {}); });
  setInterval(() => { flushOutbox().catch(() => {}); }, 60_000);

  // ============================================================
  // ★ 외부 미러 (Slack incoming webhook), 데이터 보험
  // window.__INTEGRATIONS_CONFIG.slackWebhookUrl 설정돼있어야 동작
  // ============================================================
  async function sendSlackMirror(op) {
    try {
      const cfg = window.__INTEGRATIONS_CONFIG;
      const url = cfg && cfg.slackWebhookUrl;
      if (!url) return { ok: false, reason: 'no webhook configured' };
      const ts = new Date().toISOString();
      const text =
        '[' + ts + '] ' + (op.type || 'event') +
        ' | ' + (op.email || '?') +
        ' | Day ' + (op.day || '?') +
        '\n```' + JSON.stringify(op, null, 2) + '```';
      await fetch(url, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ text }),
      });
      return { ok: true };
    } catch (e) {
      return { ok: false, error: e.message };
    }
  }

  // ============================================================
  // export
  // ============================================================
  window.OD = {
    enabled: true,
    client,
    signup,
    checkInviteCode,
    incrementCodeUsage,
    saveScenarioAtomic,
    saveNoteAtomic,
    rebuildFromHistory,
    sendMagicLink,
    verifyOtpCode,
    signInWithPassword,
    signUpWithPassword,
    setPassword,
    completeLevelup,
    getApprovedContentEdits,
    getPendingContentEdits,
    decideContentEdit,
    savePushSubscription,
    deletePushSubscription,
    getCurrentSession,
    signOut,
    updateProgress,
    syncMembership,
    logEvent,
    fetchAllUsers,
    fetchStats,
    fetchByEmail,
    fetchMembershipStatus,
    saveTfState,
    sendNotice,
    fetchNotices,
    deleteNotice,
    updateNotice,
    fetchNoticeReadCounts,
    submitFeedback,
    submitSeniorSurvey,
    fetchFeedback,
    setFeedbackResolved,
    deleteFeedback,
    replyToFeedback,
    completeFeedback,
    setFeedbackDeadline,
    saveWaitlistEntry,
    fetchWaitlist,
    cancelMembership,
    restoreMembership,
    emailExists,
    // 2026-05-19 추가
    saveSubmission,
    saveNoteRow,
    deleteNoteRow,
    saveJournalRow,
    saveDayProgress,
    saveTaskProgress,
    fetchSurveyResponses,
    setKakaoNotify,
    getKakaoNotify,
    getOfficeToday,
    addExpEvent,
    fetchWeeklyBoard,
    fetchActivityDates,
    getRetState,
    redeemWpCode,
    claimWpGrants,
    setRetState,
    peekDaySentences,
    saveSurveyResponse,
    getMentorAiFeedback,
    markNoticeRead,
    fetchNoticeReads,
    fetchUserProgress,
    fetchUserDay,
    fetchUserHealth,
    fetchUserFullData,
    fetchProgressOverview,
    sendSlackMirror,
    pushOutbox,
    flushOutbox,
  };
  console.log('[Supabase] client ready');
})();
